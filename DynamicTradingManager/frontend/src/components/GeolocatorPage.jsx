...
const GeolocatorPage = () => {
  ...
  const loadTargets = async () => {
    ...
    try {
      ...
      await loadWorkshopSources();
    } catch (error) {
      setSnackbar({ open: true, severity: 'error', message: 'Failed to load project targets.' });
    } finally {
                  ...
                  <>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                      <Button
                        ...
                        disabled={inspecting || generating || loadingTargets}
                        fullWidth
                      >
                      ...
                      <Button
                        ...
                        disabled={generating || inspecting || loadingTargets}
                        fullWidth
                      >
...
function getRegistryChipColor(status) {
  const state = status?.state || 'unknown';
