...
def _pricing_config_lua_path() -> Path:
    settings = get_server_settings()
    return (
        Path(settings.market_sense_path)
        / "Contents"
...
def _dict_to_lua(d: dict, indent: int = 1) -> str:
    lines = []
    prefix = "    " * indent
    for k, v in d.items():
        if isinstance(v, dict):
            ...
            if v:
                lines.append(_dict_to_lua(v, indent + 1))
...
def _write_pricing_config_lua(config: Dict[str, Any]) -> dict[str, Any]:
    target = _pricing_config_lua_path()
    target.parent.mkdir(parents=True, exist_ok=True)
    
    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    
    lua_body = _dict_to_lua(config, 1)
    
    lua_text = (
        "-- DynamicTrading Pricing Config\n"
        f"-- Auto-generated by ModManager on {timestamp}\n"
...
def save_pricing_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    pricing_sync = _write_pricing_config_lua(normalized)
    _CONFIG_CACHE = normalized
    _CONFIG_MTIME = CONFIG_PATH.stat().st_mtime
    result = copy.deepcopy(normalized)
    result["__lua_sync"] = lua_sync
    result["__pricing_sync"] = pricing_sync
    return result
